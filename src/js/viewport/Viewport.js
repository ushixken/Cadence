...
window.caderactCadCommands = window.CaderactCadCommandExtensions.create({ getServices: () => ({
  ...
  hitTestEditableLine: point => {
    const records = editableRecords().filter(record => record.type === "line" && !modelReader.groupForRecord(record.id))
...
for (const registration of window.CaderactLeaderCommands.registrations) window.caderactCadCommands.register(registration)
...
function selectionInteractionRecords(source=editableRecords()){
  ...
  for(const record of source){
    if(record.type!=="block-instance"){records.push(record);continue}
    try{for(const entry of window.CaderactBlockTraversal.traverse({blockDefinitions:definitions},record).entries){if(modelReader.layer(entry.record.layerId)?.visible===false)continue;const transformed=window.CaderactGeometryTransform.similarityRecord(entry.record,entry.transform);records.push(Object.freeze({...transformed,id:record.id,layerId:record.layerId}))}}catch{}
  }
  return Object.freeze(records)
}
...
function hitVisible(raw){const screen=lastKnownPointerScreen||worldToScreen(raw.x,raw.y),hit=window.CaderactSelection.hitTestRecords({screenPoint:screen,records:selectionInteractionRecords(visibleRecords()),worldToScreen,screenToWorld});return hit.hit?visibleRecords().find(record=>record.id===hit.recordId)||null:null}
function measurementMembers(record){if(record?.type!=="block-instance")return Object.freeze([record]);return Object.freeze(expandedVisibleRecords().filter(member=>member.runtimeReference?.outerRecordId===record.id&&["line","circle","arc","polyline"].includes(member.type)))}
function measurePointToInteractionRecord(point,record){let best=null;for(const member of measurementMembers(record)){const result=window.CaderactMeasurement.measurePointToRecord(point,member);if(result.valid&&(!best||result.distance<best.distance))best=Object.freeze({...result,runtimeReference:member.runtimeReference||null})}return best||Object.freeze({valid:false,reason:"unsupported-object"})}
function measureInteractionRecords(a,b){let best=null;for(const left of measurementMembers(a))for(const right of measurementMembers(b)){const result=window.CaderactMeasurement.measureRecordToRecord(left,right);if(result.valid&&(!best||result.distance<best.distance))best=Object.freeze({...result,runtimeReferenceA:left.runtimeReference||null,runtimeReferenceB:right.runtimeReference||null});if(best?.distance===0)return best}return best||Object.freeze({valid:false,reason:"unsupported-pair"})}
...
function createDistanceObjectCommandSession({setPrompt=()=>{}}={}){let source=null,hover=null,promptPresentation=createCommandPrompt("DistanceObject","Specify source point");function down(point,context={}){if(!source){source=Object.freeze({x:point.x,y:point.y});promptPresentation=createCommandPrompt("DistanceObject","Select target object");setPrompt(promptPresentation.text,promptPresentation);requestRender();return Object.freeze({status:"input-accepted",command:"DistanceObject",point:source})}const record=hitVisible(context.rawPoint||point),measurement=record&&measurePointToInteractionRecord(source,record);if(!measurement?.valid)return Object.freeze({status:"invalid-input",reason:measurement?.reason||"unsupported-object",command:"DistanceObject",message:measurement?.reason==="ellipse-distance-unsupported"?"Ellipse minimum distance is not supported.":"Select a supported object."});clearSnap();hover=null;requestRender();return Object.freeze({status:"command-completed",command:"DistanceObject",measurement,recordId:record.id,formattedMeasurement:formattedAdvanced("Distance",measurement.distance)})}function move(point,context={}){if(source){const record=hitVisible(context.rawPoint||point),m=record&&measurePointToInteractionRecord(source,record);hover=m?.valid?m:null}requestRender()}function input(value){if(source)return Object.freeze({status:"invalid-input",reason:"object-required",command:"DistanceObject",message:"Select a target object."});const parsed=resolveTypedPrecisionPoint(value,null);return parsed.status==="point-resolved"?down(parsed):Object.freeze({status:"invalid-input",reason:parsed.reason,command:"DistanceObject",message:"Enter a point as x,y."})}const cancel=()=>{clearSnap();hover=null;requestRender();return Object.freeze({status:"command-cancelled",command:"DistanceObject"})};requestRender();return Object.freeze({name:"DistanceObject",handlePointerDown:down,handlePointerMove:move,handlePointerLeave:()=>{hover=null;requestRender()},handleInput:input,finish:cancel,cancel,getMeasurementPreview:()=>hover?.targetPoint?Object.freeze({start:source,end:hover.targetPoint,distance:hover.distance,angleRadians:null}):null,hasPointerPreview:()=>true,get prompt(){return promptPresentation.text},get promptPresentation(){return promptPresentation}})}
function createMinDistanceCommandSession({setPrompt=()=>{},preselectionIds=[]}={}){let first=preselectionIds.length===1?visibleRecords().find(record=>record.id===preselectionIds[0])||null:null,hover=null,promptPresentation=createCommandPrompt("MinDist",first?"Select second object":"Select first object");function down(point,context={}){const record=hitVisible(context.rawPoint||point);if(!record)return Object.freeze({status:"input-accepted",command:"MinDist",kind:"target-miss"});if(!first){first=record;promptPresentation=createCommandPrompt("MinDist","Select second object");setPrompt(promptPresentation.text,promptPresentation);requestRender();return Object.freeze({status:"input-accepted",command:"MinDist",recordId:record.id})}if(record.id===first.id)return Object.freeze({status:"invalid-input",reason:"same-object",command:"MinDist",message:"Select a different second object."});const measurement=measureInteractionRecords(first,record);if(!measurement.valid)return Object.freeze({status:"invalid-input",reason:measurement.reason,command:"MinDist",message:first.type==="ellipse"||record.type==="ellipse"?"Ellipse minimum distance is not supported.":"This object pair is not supported."});clearSnap();hover=null;requestRender();return Object.freeze({status:"command-completed",command:"MinDist",measurement,formattedMeasurement:formattedAdvanced("Minimum Distance",measurement.distance)})}function move(point,context={}){if(first){const record=hitVisible(context.rawPoint||point),m=record&&record.id!==first.id?measureInteractionRecords(first,record):null;hover=m?.valid?m:null}requestRender()}const cancel=()=>{clearSnap();hover=null;requestRender();return Object.freeze({status:"command-cancelled",command:"MinDist"})};requestRender();return Object.freeze({name:"MinDist",handlePointerDown:down,handlePointerMove:move,handlePointerLeave:()=>{hover=null;requestRender()},finish:cancel,cancel,usesResolvedPoint:false,getMeasurementPreview:()=>hover?.pointA&&hover?.pointB?Object.freeze({start:hover.pointA,end:hover.pointB,distance:hover.distance,angleRadians:null}):null,hasPointerPreview:()=>true,get prompt(){return promptPresentation.text},get promptPresentation(){return promptPresentation}})}
...
function createBlockCommandSession({setPrompt=()=>{}}={}){
  const eligible=new Set(["line","circle","arc","ellipse","polyline","region","hatch","text","dimension-linear","dimension-angular","dimension-radial","dimension-ordinate","dimension-arc-length","dimension-center-mark","dimension-center-line","dimension-leader","dimension-multileader","block-instance"])
...
function createBlockEditCommandSession({setPrompt=()=>{}}={}){
  ...
  function choose(value){const key=String(value??"").trim().toLowerCase(),matches=modelReader.blockDefinitions().filter(item=>item.name.toLowerCase()===key);if(matches.length!==1)return Object.freeze({status:"invalid-input",reason:key?"unknown-definition":"definition-required",command:"BlockEdit",message:key?"Unknown Block Definition":"Enter a Block Definition name"});definition=matches[0];workspace=window.CaderactBlockEditWorkspace.create({definition,copyRecord:recordGateway.copyWithFreshIdentity,replaceDefinition:blockDefinitionGateway.replace,getDefinitions:modelReader.blockDefinitions});phase="edit";update("Select definition members; use Move, Copy, Rotate, Scale, or Delete");return Object.freeze({status:"input-accepted",command:"BlockEdit",kind:"definition",definitionId:definition.id})}
  ...
  function handlePointerDown(point,context={}){if(!workspace)return Object.freeze({status:"invalid-input",reason:"definition-required",command:"BlockEdit"});const working=workspace.snapshot(),records=working.recordOrder.map(id=>working.records[id]).filter(record=>record.type!=="block-instance"),screenPoint=lastKnownPointerScreen||worldToScreen(context.rawPoint?.x??point.x,context.rawPoint?.y??point.y),hit=window.CaderactSelection.hitTestRecords({screenPoint,records,worldToScreen,screenToWorld});if(!hit.hit){workspace.clearSelection();requestRender();return Object.freeze({status:"input-accepted",command:"BlockEdit",kind:"selection",recordIds:workspace.selectedIds})}context.toggle?workspace.toggle(hit.recordId):workspace.select([hit.recordId]);requestRender();return Object.freeze({status:"input-accepted",command:"BlockEdit",kind:"selection",recordIds:workspace.selectedIds})}
  ...
  requestRender();return Object.freeze({name:"BlockEdit",finish:()=>phase==="edit"?apply():Object.freeze({status:"invalid-input",reason:"definition-required",command:"BlockEdit",message:"Enter a Block Definition name"}),cancel,handleInput,handleOption,handlePointerDown,getMovePreview,hasPointerPreview:()=>false,usesResolvedPoint:false,get workspace(){return workspace},get definition(){return definition},get phase(){return phase},get options(){return options()},get prompt(){return promptPresentation.text},get promptPresentation(){return promptPresentation}})
}
...
function resolvePointerSnap(point, { excludedFeatureIds = [], excludedRecordIds = [], transientCandidates = [], referencePoint = null, bypass = false } = {}) {
  ...
  activeSnapResult = snapResolver.resolve({
    rawWorldPoint: point,
    worldToScreen,
    records: expandedVisibleRecords(),
    transientCandidates,
...
function onViewportPointerDown(event) {
  ...
  if (session?.handlePointerDown && !session.isSelectionPhase) {
    ...
    window.caderactCommandRouter.submitActivePointer(snap.point, { snap, rawPoint: screenToWorld(point.x, point.y), toggle:(event.ctrlKey||event.metaKey)&&!(event.ctrlKey&&event.metaKey) })
    refreshDynamicInput()
    return
  }
  if (!session) {

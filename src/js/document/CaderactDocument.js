...
(() => {
  ...
  function createStore({ document: initialDocument, initiallySaved = false } = {}) {
    ...
    const recordGateway = Object.freeze({
      ...
      createHatchFromLoops(inputs){const region=this.createRegionFromLoops(inputs);return freeze({...region,type:"hatch",pattern:{kind:"solid"}})},
      createHatch(boundaries){if(boundaries.length&&boundaries.every(record=>record?.type==="region"||record?.type==="hatch")){if(boundaries.length!==1)throw new Error("Create one Hatch per semantic Region");return this.createHatchFromLoops(boundaries[0].loops.map(loop=>loop.edges))}return this.createHatchFromLoops(window.CaderactRegionGeometry.canonicalizeSources(boundaries).map(loop=>loop.edges))},

...
test('registry is the single deterministic autocomplete and routing source', async () => {
  const b = await browser();
  assert.deepEqual(b.read('window.caderactCommandRegistry.commands().filter(command=>!["ArcLength","CenterLine","CenterMark","DimBaseline","DimContinue","Ordinate"].includes(command.name)).map(command=>({name:command.name,aliases:command.aliases}))'), [

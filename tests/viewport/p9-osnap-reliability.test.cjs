...
test('raw CAD crosshair stays separate while marker, preview, HUD, and acceptance share the final semantic coordinate',async()=>{const b=await browser();b.run(`recordGateway.createAll([recordGateway.createLine({x:0,y:0},{x:20,y:0})])`);modes(b,['endpoint','nearest']);b.launch('Line');b.run(`window.caderactCommandRouter.submitActiveInput('-10,10')`);const result=move(b,406,299);b.flush();assert.deepEqual(result.point,{x:0,y:0});assert.deepEqual(b.read('window.caderactCommandRouter.activeSession.draft.preview().end'),{x:0,y:0});assert.deepEqual(b.read('window.caderactViewport.getDynamicInputState().fields.map(x=>x.value)'),['14.142','-45.00°']);assert.deepEqual({x:b.renders.at(-1).snapOverlay.point.x,y:b.renders.at(-1).snapOverlay.point.y},{x:400,y:300});assert.deepEqual(b.read('window.caderactViewport.getInteractionVisualState()'),{visible:true,x:416,y:309,mode:'point',snapAcquired:true,available:true,navigating:false});assert.deepEqual(b.read('(({x,y})=>worldToScreen(x,y))(activeSnapResult.point)'),{x:400,y:300});b.point(406,299);assert.deepEqual(b.read('modelReader.lines().at(-1).end'),{x:0,y:0,featureId:b.read('modelReader.lines().at(-1).end.featureId')})});

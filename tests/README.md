...
- `commands/line.test.cjs`: first point, independent segments, live preview, completion, cancellation preserving earlier geometry, empty sessions.
...
The harness executes unchanged production files in a fresh VM for each test. It reads existing lexical state for assertions instead of adding test-only production exports. Those internal assertions will need adaptation when Line migrates to drafts; user-visible input and output expectations must remain equivalent. Mathematical tests call the actual production functions, not reimplemented formulas.
...
- Preview is removed on pointer leave; completed geometry remains.
- Enter used to launch a suggestion is not also consumed as Line completion.
- Current Escape cancellation truncates the current session's completed-lines suffix. Stage 1 captures this pre-transaction implementation deliberately.
...
- No persistence system exists here; preview separation is tested against completed geometry, not a saved file.
...
Stage 1 adds no Document Model, transactions, Line migration, new navigation bindings, or renderer redesign.

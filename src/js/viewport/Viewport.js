...
function onViewportPointerUp(event) {
  if(selectionBox.isPending&&selectionBox.snapshot().pointerId===event.pointerId){
    ...
    if(box.active){const session=getActiveCommandSession(),selectionRecords=session?.isSelectionPhase&&(session.name==="Trim"||session.name==="Extend")?visibleRecords():editableRecords(),outcome=window.CaderactSelectionBox.query({start:box.start,current:box.current,records:selectionRecords,worldToScreen}),matched=new Set(outcome.recordIds),logical=[]

...
test('panel controls expose independent accessible state and persistence keeps visible/locked exactly',async()=>{
  const b=await browser();seed(b);let row=b.layersList.children.find(row=>row.dataset.layerId===b.read('window.__layer.id'));assert.equal(row.children[1].getAttribute('aria-pressed'),'true');assert.equal(row.children[2].getAttribute('aria-pressed'),'false');
  b.emit(row.children[1],'click');row=b.layersList.children.find(row=>row.dataset.layerId===b.read('window.__layer.id'));assert.equal(row.children[1].getAttribute('aria-pressed'),'false');assert.notEqual(b.read('modelReader.snapshot().currentLayerId'),b.read('window.__layer.id'));
  b.emit(row.children[1],'click');b.emit(row.children[2],'click');const serialized=b.run('window.CaderactPersistence.serializeDocument(modelReader.snapshot())');b.run(`window.__loaded=window.CaderactPersistence.loadStore(${JSON.stringify(serialized)})`);assert.equal(b.read('window.__loaded.reader.layer(window.__layer.id).visible'),true);assert.equal(b.read('window.__loaded.reader.layer(window.__layer.id).locked'),true);assert.equal(JSON.parse(serialized).fileVersion,3);
});

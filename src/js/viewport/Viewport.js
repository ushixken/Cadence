...
function createObjectMeasurementCommandSession(name,{preselectionIds=[]}={}){
  const specs={Length:{types:new Set(["line","circle","arc","polyline"]),fields:{line:"length",circle:"circumference",arc:"arcLength",polyline:"length"}},Radius:{types:new Set(["circle","arc"]),fields:{circle:"radius",arc:"radius"}},Diameter:{types:new Set(["circle","arc"]),fields:{circle:"diameter",arc:"diameter"}}},spec=specs[name]
  const promptPresentation=createCommandPrompt(name,`Select ${name==="Length"?"a measurable object":"a Circle or Arc"}`)
  ...
  function handlePointerDown(point,context={}){const raw=context.rawPoint||point,screenPoint=lastKnownPointerScreen||worldToScreen(raw.x,raw.y),hit=window.CaderactSelection.hitTestRecords({screenPoint,records:visibleRecords(),worldToScreen}),record=hit.hit?recordById(hit.recordId):null;if(!record)return Object.freeze({status:"input-accepted",command:name,kind:"target-miss"});if(!spec.types.has(record.type))return Object.freeze({status:"invalid-input",reason:"unsupported-object",command:name,message:`${name} supports ${name==="Length"?"Line, Circle, Arc, or Polyline":"Circle or Arc"}`});clearSnap();requestRender();return complete(record)}
  ...
  const preselected=preselectionIds.length===1?recordById(preselectionIds[0]):null,activationOutcome=preselected&&spec.types.has(preselected.type)?complete(preselected):null
  requestRender()

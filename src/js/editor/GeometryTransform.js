...
(() => {
  ...
  const dimensionPointKeys=record=>record.type==="dimension-linear"?["firstPoint","secondPoint","dimensionLinePoint"]:record.type==="dimension-angular"?["firstRayPoint","vertex","secondRayPoint","dimensionArcPoint"]:record.type==="dimension-radial"?["centerPoint","dimensionPoint","leaderPoint"]:record.type==="dimension-ordinate"?["datumPoint","featurePoint","leaderPoint"]:record.type==="dimension-arc-length"?["centerPoint","startPoint","endPoint","dimensionArcPoint"]:record.type==="dimension-center-mark"?["centerPoint","sizePoint"]:record.type==="dimension-center-line"?["firstPoint","secondPoint"]:null
  ...
  function transformDimension(record,transform){if(record.type==="dimension-leader")return Object.freeze({...record,vertices:Object.freeze(record.vertices.map(transform)),textPosition:transform(record.textPosition)});if(record.type==="dimension-multileader")return Object.freeze({...record,branches:Object.freeze(record.branches.map(branch=>Object.freeze({...branch,vertices:Object.freeze(branch.vertices.map(transform))}))),textPosition:transform(record.textPosition)});const keys=dimensionPointKeys(record);if(!keys)return null;const result={...record};for(const key of keys)result[key]=transform(record[key]);if(record.textPositionMode==="manual"&&record.manualTextPosition)result.manualTextPosition=transform(record.manualTextPosition);return Object.freeze(result)}

...
test('a stale transaction after Undo preserves its existing Redo branch', async () => {
  ...
  const custom = b.run(`(() => {
    ...
    let state = { id:'history_stale_doc', name:'Test', formatVersion:1, geometry:{ objects:{} }, layers, defaultLayerId:${JSON.stringify(layerId)}, currentLayerId:${JSON.stringify(layerId)} };
    const hooks = {};
...
test('history integrity failure never partially publishes or advances cursor, state, or revision', async () => {
  ...
  const custom = b.run(`(() => {
    const layers = { ${JSON.stringify(layerId)}: { id:${JSON.stringify(layerId)}, name:'Default', visible:true, locked:false } };
    let state = { id:'history_integrity_doc', name:'Test', formatVersion:1, geometry:{ objects:{} }, layers, defaultLayerId:${JSON.stringify(layerId)}, currentLayerId:${JSON.stringify(layerId)} };
    const controller = window.DocumentController.createController({
      getDocument: () => state,

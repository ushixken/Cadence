    ...
    function tableFrom(items, label, canonicalize) {
      ...
      for (const item of items) {
        ...
        if (label === "record") {
          ...
          } else if(!legacy&&!version2&&item.type==="region"){
            rejectUnknown(item,fields.region,"Region record");if(!Array.isArray(item.loops))invalid("Region loops must be an array");for(const loop of item.loops){if(!isRecord(loop))invalid("Region loop must be an object");rejectUnknown(loop,fields.regionLoop,"Region loop");if(!Array.isArray(loop.edges))invalid("Region edges must be an array");for(const edge of loop.edges){if(!isRecord(edge))invalid("Region edge must be an object");rejectUnknown(edge,fields.regionEdge,"Region edge");for(const key of ["start","end"])if(edge[key])rejectUnknown(edge[key],fields.endpoint,`Region edge ${key}`);for(const key of ["center","majorAxis"])if(edge[key])rejectUnknown(edge[key],fields.coordinate,`Region edge ${key}`)}}

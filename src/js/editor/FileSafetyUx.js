...
(() => {
  ...
  function create({ recoveryValidation, session, fileState, autosave, viewport, commandRouter, getFileActions = () => window.caderactFiles,
    ...
    let candidates=[],selectedIndex=0,unsavedResolve=null,focusBefore=null,unsubscribeHistory=null,autosaveWarning=false
    ...
    function presentAutosave(outcome){if(outcome?.status==="autosave-completed"){autosaveWarning=false;return}if(outcome?.status==="autosave-failed"&&!autosaveWarning){autosaveWarning=true;announce("Automatic recovery could not be updated. Save your drawing manually.","error")}}
    ...
    session.subscribe(()=>{bindController();renderSaveState()});fileState.subscribe(()=>renderSaveState());autosave?.subscribe?.(presentAutosave);bindController();renderSaveState()

...
function createObjectMeasurementCommandSession(name,{preselectionIds=[]}={}){
  const specs={Length:{types:new Set(["line","circle","arc","polyline"]),fields:{line:"length",circle:"circumference",arc:"arcLength",polyline:"length"}},Radius:{types:new Set(["circle","arc"]),fields:{circle:"radius",arc:"radius"}},Diameter:{types:new Set(["circle","arc"]),fields:{circle:"diameter",arc:"diameter"}},Area:{types:new Set(["circle","ellipse","polyline"]),fields:{circle:"area",ellipse:"area",polyline:"area"}},Perimeter:{types:new Set(["circle","polyline"]),fields:{circle:"perimeter",polyline:"perimeter"}}},spec=specs[name]
...
function hitVisible(raw){const screen=lastKnownPointerScreen||worldToScreen(raw.x,raw.y),hit=window.CaderactSelection.hitTestRecords({screenPoint:screen,records:visibleRecords(),worldToScreen});return hit.hit?visibleRecords().find(record=>record.id===hit.recordId)||null:null}
...
function createRegionCommandSession({setPrompt=()=>{}}={}){
  let promptPresentation=createCommandPrompt("Region","Select closed boundary objects, then press Enter")
  function finish(){const ids=selection.orderedIds(),byId=new Map(editableRecords().map(record=>[record.id,record])),sources=ids.map(id=>byId.get(id)).filter(Boolean);if(!sources.length)return Object.freeze({status:"invalid-input",reason:"empty-selection",command:"Region",message:"Select closed boundary objects"});let record;try{record=recordGateway.createRegion(sources)}catch(error){setPrompt(error.message,promptPresentation);return Object.freeze({status:"invalid-input",reason:"invalid-boundary",command:"Region",message:error.message})}const outcome=recordGateway.createAll([record]);if(outcome.status!=="committed")return Object.freeze({status:"invalid-input",reason:"commit-failed",command:"Region",message:"Unable to create Region",outcome});selection.applyRecordIds([record.id]);clearSnap();requestRender();return Object.freeze({status:"command-completed",command:"Region",recordId:record.id,outcome})}
  function cancel(){clearSnap();requestRender();return Object.freeze({status:"command-cancelled",command:"Region"})}
  requestRender();return Object.freeze({name:"Region",finish,cancel,handlePointerLeave:()=>requestRender(),get isSelectionPhase(){return true},get prompt(){return promptPresentation.text},get promptPresentation(){return promptPresentation}})
}
...
function selectAllCommittedGeometry() {
  ...
  const selectableTypes = new Set(["line", "circle", "arc", "ellipse", "polyline", "region", "text", "dimension-linear", "dimension-angular", "dimension-radial"])

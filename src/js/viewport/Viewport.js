...
function createHatchCommandSession({setPrompt=()=>{}}={}){
  let mode="select",promptPresentation=createCommandPrompt("Hatch","Select closed boundaries or Regions, then press Enter")
...
function selectAllCommittedGeometry() {
  ...
  const selectableTypes = new Set(["line", "circle", "arc", "ellipse", "polyline", "region", "hatch", "text", "dimension-linear", "dimension-angular", "dimension-radial"])
...
function prepareContextSelection(screenPoint){
  ...
  const editableHit=window.CaderactSelection.hitTestRecords({screenPoint,records:editableRecords(),worldToScreen,screenToWorld})
  ...
  const visibleHit=window.CaderactSelection.hitTestRecords({screenPoint,records:visibleRecords(),worldToScreen,screenToWorld})

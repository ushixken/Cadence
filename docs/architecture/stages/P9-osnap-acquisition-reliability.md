...
Caderact hides the native pointer and draws its own CAD crosshair. During an acquired snap, that crosshair is projected to the same authoritative final point used by the snap marker, preview, Dynamic Input, and pointer acceptance. The physical OS pointer is never warped; leaving the snap releases the drawn crosshair back to raw pointer feedback.

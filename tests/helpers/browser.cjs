...
async function browser({ commands = true, realRenderer = false, gpu } = {}) {
  ...
  for (const el of [layersList,layerCreateButton,layerAssignButton,layerUnisolateButton,layersTab,propertiesTab,layersView,propertiesView,propertiesContent,contextMenu,gridSnapButton,orthoButton,polarButton,trackButton]) { el.parent=document; el.owner=document; }

...
(() => {
  ...
  function canonicalRecord(record) {
    ...
    if(record.type==="dimension-ordinate")return{id:record.id,type:record.type,layerId:record.layerId,...properties(record),axis:record.axis,datumPoint:feature(record.datumPoint),featurePoint:feature(record.featurePoint),leaderPoint:feature(record.leaderPoint),textOverride:record.textOverride,dimensionStyleId:record.dimensionStyleId}
    if(record.type==="dimension-arc-length")return{id:record.id,type:record.type,layerId:record.layerId,...properties(record),centerPoint:feature(record.centerPoint),startPoint:feature(record.startPoint),endPoint:feature(record.endPoint),dimensionArcPoint:feature(record.dimensionArcPoint),sweep:record.sweep,sourceRecordId:record.sourceRecordId,textOverride:record.textOverride,dimensionStyleId:record.dimensionStyleId}
      ...
      for (const item of items) {
        ...
        if (label === "record"||label==="definition record") {
          if (item.type === "line") {
            ...
            rejectUnknown(item,fields.dimensionOrdinate,"Ordinate dimension record");for(const key of ["datumPoint","featurePoint","leaderPoint"]){if(!isRecord(item[key]))invalid(`Ordinate dimension ${key} must be an object`);rejectUnknown(item[key],fields.endpoint,`Ordinate dimension ${key}`)}
          } else if(!legacy&&!version2&&item.type==="dimension-arc-length"){
            rejectUnknown(item,fields.dimensionArcLength,"Arc-length dimension record");for(const key of ["centerPoint","startPoint","endPoint","dimensionArcPoint"]){if(!isRecord(item[key]))invalid(`Arc-length dimension ${key} must be an object`);rejectUnknown(item[key],fields.endpoint,`Arc-length dimension ${key}`)}

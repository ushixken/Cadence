(() => {
  ...
  function dimensionPoint(record,grip){if(!record?.type?.startsWith("dimension-"))return null;if(grip.kind==="textPosition")return record.textPosition?.featureId===grip.featureId?record.textPosition:null;const leader=/^vertex:(\d+)$/.exec(grip.kind),multi=/^branch:(\d+):(\d+)$/.exec(grip.kind);if(leader)return record.vertices?.[Number(leader[1])]?.featureId===grip.featureId?record.vertices[Number(leader[1])]:null;if(multi)return record.branches?.[Number(multi[1])]?.vertices?.[Number(multi[2])]?.featureId===grip.featureId?record.branches[Number(multi[1])].vertices[Number(multi[2])]:null;return record[grip.kind]?.featureId===grip.featureId?record[grip.kind]:null}
  function dimensionReplacement(record,grip,point){const replacement=value=>({...value,x:point.x,y:point.y});if(grip.kind==="textPosition")return{...record,textPosition:replacement(record.textPosition)};const leader=/^vertex:(\d+)$/.exec(grip.kind),multi=/^branch:(\d+):(\d+)$/.exec(grip.kind);if(leader){const index=Number(leader[1]);return{...record,vertices:record.vertices.map((value,current)=>current===index?replacement(value):value)}}if(multi){const branchIndex=Number(multi[1]),vertexIndex=Number(multi[2]);return{...record,branches:record.branches.map((branch,current)=>current===branchIndex?{...branch,vertices:branch.vertices.map((value,index)=>index===vertexIndex?replacement(value):value)}:branch)}}return{...record,[grip.kind]:replacement(record[grip.kind])}}
  function validDimension(record){if(record.type==="dimension-leader")return record.vertices.length>=2;if(record.type==="dimension-multileader")return record.branches.length>0&&record.branches.every(branch=>branch.vertices.length>=2);if(record.type==="dimension-linear")return Math.hypot(record.secondPoint.x-record.firstPoint.x,record.secondPoint.y-record.firstPoint.y)>0;if(record.type==="dimension-angular"){const rays=window.CaderactAngularDimensionDraftSession.validateRays(record.firstRayPoint,record.vertex,record.secondRayPoint),radius=Math.hypot(record.dimensionArcPoint.x-record.vertex.x,record.dimensionArcPoint.y-record.vertex.y);return rays.valid&&radius>0}if(record.type==="dimension-radial")return Math.hypot(record.dimensionPoint.x-record.centerPoint.x,record.dimensionPoint.y-record.centerPoint.y)>0;if(record.type==="dimension-ordinate")return Number.isFinite(record.featurePoint?.x)&&Number.isFinite(record.leaderPoint?.x);if(record.type==="dimension-arc-length")return Math.abs(record.sweep)>0&&Math.hypot(record.startPoint.x-record.centerPoint.x,record.startPoint.y-record.centerPoint.y)>0;if(record.type==="dimension-center-mark")return Math.hypot(record.sizePoint.x-record.centerPoint.x,record.sizePoint.y-record.centerPoint.y)>0;if(record.type==="dimension-center-line")return Math.hypot(record.secondPoint.x-record.firstPoint.x,record.secondPoint.y-record.firstPoint.y)>0;return false}

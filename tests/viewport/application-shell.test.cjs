...
test('UX2R3 keeps command history inside workspace as a non-layout overlay', () => {
  ...
  assert.match(css, /\.editor-workspace\{position:relative;--right-dock-width:288px\}/)
  ...
  assert.match(css, /\.command-history-region\{[^}]*max-width:calc\(100% - var\(--right-dock-width\) - 56px\)/s)

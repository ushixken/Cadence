...
async function browser({ commands = true, realRenderer = false, gpu } = {}) {
  ...
  const snapWrap = new Element(); const snapTrigger = new Element('button'); const snapMenu = new Element(); const snapEnabled = new Element('input'); const insertionSnapOption = new Element('input'); const extensionTrackingOption = new Element('input'); const snapDependent = new Element();
  ...
  snapTrigger.parent=snapWrap; snapMenu.parent=snapWrap; snapEnabled.parent=snapMenu; insertionSnapOption.parent=snapDependent; extensionTrackingOption.parent=snapMenu; snapDependent.parent=snapMenu;
  ...
  for(const el of [snapWrap,snapTrigger,snapMenu,snapEnabled,insertionSnapOption,extensionTrackingOption,snapDependent,unitsWrap,unitsTrigger,unitsMenu,unitValue,...unitOptions]) { el.owner=document; if(!el.parent) el.parent=document; }
  snapEnabled.checked=true; snapEnabled.dataset.snapMode='object'; insertionSnapOption.checked=true; insertionSnapOption.dataset.snapMode='insertion'; extensionTrackingOption.checked=true; unitValue.textContent='mm'; unitOptions[0].classList.add('is-selected');
  ...
  document.querySelectorAll = selector => ({ '.menu-items > li > button':[fileMenuTrigger,toolsMenuTrigger,editMenuTrigger], '.snap-dependent input':[insertionSnapOption], '[data-snap-mode]':[snapEnabled,insertionSnapOption], '.footer-tool':[gridSnapButton,orthoButton,polarButton,trackButton], '.unit-option':unitOptions })[selector] || [];
  ...
  return { window, document, viewportHost,editorWorkspace,layersPanel,rightDockResize, contextMenu, gridSnapButton, orthoButton, polarButton, trackButton, snapTrigger, snapEnabled, get canvas() { return canvas; }, input, suggestions, commandHistory, commandPrompt, undoButton, redoButton, fileNewButton, fileOpenButton, fileSaveButton, fileSaveAsButton, fileImportDxfButton, fileExportDxfButton, fileMenu, fileMenuTrigger, fileMenuDropdown, toolsMenu,toolsMenuTrigger,measureMenuDropdown,measureMenuItems, editMenuTrigger, unitsTrigger, unitsMenu, unitValue, unitOptions, layersList, layerCreateButton, layerAssignButton, layerUnisolateButton, groupsList,groupCreateButton,blocksList,blockCreateButton,blockPurgeButton,blockEditContext,blockEditName,blockEditSave,blockEditDiscard,layersTab,groupsTab,blocksTab,propertiesTab,layersView,groupsView,blocksView,propertiesView,propertiesContent,context, run, load, emit, renders, sizes, fakeRenderer, drawCalls, observerStats,

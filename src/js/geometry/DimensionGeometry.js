...
(() => {
  ...
  const textPoint=(record,automatic)=>record.textPositionMode==="manual"&&record.manualTextPosition?point(record.manualTextPosition):point(automatic)
  const textGrips=record=>record.textPositionMode==="manual"&&record.manualTextPosition?[Object.freeze({kind:"manualTextPosition",point:point(record.manualTextPosition),featureId:record.manualTextPosition.featureId||null})]:[]
  ...
  function angular(record,style,units){
    ...
    const displayTextPoint=record.textPositionMode==="manual"&&record.manualTextPosition?point(record.manualTextPosition):textPoint
    return Object.freeze({supported:true,measurement,lines,arcs:Object.freeze([arc]),triangles,text:Object.freeze({point:displayTextPoint,rotation:readableRotation(midAngle+Math.PI/2),value:window.CaderactDimensionFormatter.format(measurement,style,units,record),height:style.textHeight}),hitPrimitives:Object.freeze({lines,arcs:Object.freeze([arc])}),bounds:Object.freeze({minX:Math.min(v.x-radius-style.extensionBeyond,displayTextPoint.x),minY:Math.min(v.y-radius-style.extensionBeyond,displayTextPoint.y),maxX:Math.max(v.x+radius+style.extensionBeyond,displayTextPoint.x),maxY:Math.max(v.y+radius+style.extensionBeyond,displayTextPoint.y)}),grips:Object.freeze([...grips,...textGrips(record)])})
  }
  function radial(record,style,units){
    ...
    const displayTextPoint=record.textPositionMode==="manual"&&record.manualTextPosition?point(record.manualTextPosition):textPoint
    return Object.freeze({supported:true,measurement,lines,arcs:Object.freeze([]),triangles,text:Object.freeze({point:displayTextPoint,rotation:readableRotation(Math.atan2(leaderDirection.y,leaderDirection.x)),value:window.CaderactDimensionFormatter.format(measurement,style,units,record),height:style.textHeight}),hitPrimitives:Object.freeze({lines,arcs:Object.freeze([])}),bounds:Object.freeze({minX:Math.min(center.x,tip.x,leader.x,opposite.x,displayTextPoint.x),minY:Math.min(center.y,tip.y,leader.y,opposite.y,displayTextPoint.y),maxX:Math.max(center.x,tip.x,leader.x,opposite.x,displayTextPoint.x),maxY:Math.max(center.y,tip.y,leader.y,opposite.y,displayTextPoint.y)}),grips:Object.freeze([...grips,...textGrips(record)])})
  }
  ...
  function ordinate(record,style,units){const a=record.featurePoint,l=record.leaderPoint,vertical=record.axis==="x",elbow=vertical?{x:l.x,y:a.y}:{x:a.x,y:l.y},lines=[[point(a),point(elbow)],[point(elbow),point(l)]],measurement=measure(record);return result({measurement,lines,text:Object.freeze({point:textPoint(record,l),rotation:0,value:window.CaderactDimensionFormatter.format(measurement,style,units,record),height:style.textHeight}),grips:[grip(record,"datumPoint"),grip(record,"featurePoint"),grip(record,"leaderPoint"),...textGrips(record)]})}
  function arcLength(record,style,units){const center=record.centerPoint,r=Math.hypot(record.dimensionArcPoint.x-center.x,record.dimensionArcPoint.y-center.y);if(!(r>0))return Object.freeze({supported:false,measurement:measure(record),reason:"degenerate-arc-length-geometry"});const startAngle=Math.atan2(record.startPoint.y-center.y,record.startPoint.x-center.x),arc=Object.freeze({center:point(center),start:point({x:center.x+Math.cos(startAngle)*r,y:center.y+Math.sin(startAngle)*r}),radius:r,startAngle,sweep:record.sweep}),mid=startAngle+record.sweep/2,measurement=measure(record),automatic={x:center.x+Math.cos(mid)*(r+style.textGap),y:center.y+Math.sin(mid)*(r+style.textGap)};return result({measurement,arcs:[arc],text:Object.freeze({point:textPoint(record,automatic),rotation:readableRotation(mid+Math.PI/2),value:"⌒"+window.CaderactDimensionFormatter.format(measurement,style,units,record),height:style.textHeight}),grips:[grip(record,"centerPoint"),grip(record,"startPoint"),grip(record,"endPoint"),grip(record,"dimensionArcPoint"),...textGrips(record)]})}
  ...
  function leaderPresentation(record,style){const branches=record.type==="dimension-leader"?[{featureId:null,vertices:record.vertices}]:record.branches,lines=[],triangles=[],grips=[];for(let branchIndex=0;branchIndex<branches.length;branchIndex++){const branch=branches[branchIndex],vertices=branch.vertices;for(let index=1;index<vertices.length;index++)lines.push([point(vertices[index-1]),point(vertices[index])]);triangles.push(arrow(point(vertices[0]),point(vertices[1]),style.arrowSize));for(let index=0;index<vertices.length;index++)grips.push(Object.freeze({kind:record.type==="dimension-leader"?`vertex:${index}`:`branch:${branchIndex}:${index}`,point:point(vertices[index]),featureId:vertices[index].featureId||null,branchFeatureId:branch.featureId||null}))}const text=record.textPosition,last=branches[0].vertices.at(-1),direction=text.x>=last.x?1:-1,landingEnd={x:text.x-direction*style.textGap,y:text.y};for(const branch of branches){const end=branch.vertices.at(-1);if(Math.hypot(end.x-landingEnd.x,end.y-landingEnd.y)>0)lines.push([point(end),point(landingEnd)])}grips.push(Object.freeze({kind:"textPosition",point:point(text),featureId:text.featureId||null}));return result({lines,triangles,text:Object.freeze({point:point(text),rotation:0,value:record.textOverride||"",height:style.textHeight}),grips})}
  function derive(record, style, units) {
    ...
    if(record.type==="dimension-leader"||record.type==="dimension-multileader")return leaderPresentation(record,style)

  ...
  function validateDocument(value) {
    ...
    if (!isRecord(objects)) errors.push("Invalid object table")
      ...
      if (record.type === "line") {
        ...
        closedShape(record,schema.region,"Region")
        if(!Array.isArray(record.loops))errors.push("Region: loops must be an array")
        else for(const loop of record.loops){closedShape(loop,schema.regionLoop,"Region loop");identity(loop?.featureId,"Region loop feature");if(!Array.isArray(loop?.edges))errors.push("Region loop: edges must be an array");else for(const edge of loop.edges){closedShape(edge,schema.regionEdge,"Region edge");identity(edge?.featureId,"Region edge feature");for(const key of ["start","end"])if(edge[key]){point(edge[key],`Region edge ${key}`);closedShape(edge[key],schema.endpoint,`Region edge ${key}`);identity(edge[key].featureId,`Region edge ${key} feature`)}}}
        for(const error of window.CaderactRegionGeometry.validate(record))errors.push(error)
  ...
  function createStore({ document: initialDocument, initiallySaved = false } = {}) {
    ...
    if (initialDocument !== undefined) {
      ...
      for (const record of Object.values(candidate.geometry.objects)) {
        ...
        else if(record.type==="region")for(const loop of record.loops){allocated.add(loop.featureId);for(const edge of loop.edges){allocated.add(edge.featureId);if(edge.start?.featureId)allocated.add(edge.start.featureId);if(edge.end?.featureId)allocated.add(edge.end.featureId)}}
      }
    ...
    const recordGateway = Object.freeze({
      ...
      copyWithFreshIdentity(record) {
        ...
        if(record.type==="region")return freeze({...record,id:newId(),loops:record.loops.map(loop=>({...loop,featureId:newId(),edges:loop.edges.map(edge=>({...edge,featureId:newId(),...(edge.start?{start:{...edge.start,featureId:newId()}}:{}),...(edge.end?{end:{...edge.end,featureId:newId()}}:{})}))}))})

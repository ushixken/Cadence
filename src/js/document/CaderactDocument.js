...
(() => {
  ...
  const DIMENSION_CONTROL_FIELDS=Object.freeze(["prefixOverride","suffixOverride","toleranceMode","toleranceUpper","toleranceLower","textPositionMode","manualTextPosition"])
  const DIMENSION_CONTROL_TYPES=new Set(["dimension-linear","dimension-angular","dimension-radial","dimension-ordinate","dimension-arc-length"])
  ...
  const V3_FIELDS=Object.freeze({...V2_FIELDS,persistedDocument:fields([...V1_FIELDS.persistedDocument,"layerOrder","dimensionStyles","currentDimensionStyleId","groups","nextGroupNumber","blockDefinitions"]),document:fields([...V1_FIELDS.document,"layerOrder","dimensionStyles","dimensionStyleOrder","currentDimensionStyleId","groups","nextGroupNumber","blockDefinitions"]),group:fields(["id","name","memberIds"]),blockDefinition:fields(["id","name","basePoint","records","recordOrder"]),blockInstance:fields(["id","type","layerId","color","linetype","lineweight","definitionId","insertionPoint","rotation","scale","mirrored"]),dimensionStyle:fields(["id","name",...Object.keys(DEFAULT_DIMENSION_STYLE)]),dimensionLinear:fields([...V2_FIELDS.dimensionLinear,"dimensionStyleId",...DIMENSION_CONTROL_FIELDS]),dimensionAngular:fields([...V2_FIELDS.dimensionAngular,"dimensionStyleId",...DIMENSION_CONTROL_FIELDS]),dimensionRadial:fields([...V2_FIELDS.dimensionRadial,"dimensionStyleId",...DIMENSION_CONTROL_FIELDS]),dimensionOrdinate:fields(["id","type","layerId","color","linetype","lineweight","axis","datumPoint","featurePoint","leaderPoint","textOverride","dimensionStyleId",...DIMENSION_CONTROL_FIELDS]),dimensionArcLength:fields(["id","type","layerId","color","linetype","lineweight","centerPoint","startPoint","endPoint","dimensionArcPoint","sweep","sourceRecordId","textOverride","dimensionStyleId",...DIMENSION_CONTROL_FIELDS]),dimensionCenterMark:fields(["id","type","layerId","color","linetype","lineweight","centerPoint","sizePoint","sourceRecordId","dimensionStyleId"]),dimensionCenterLine:fields(["id","type","layerId","color","linetype","lineweight","firstPoint","secondPoint","extension","sourceRecordIds","dimensionStyleId"]),dimensionLeader:fields(["id","type","layerId","color","linetype","lineweight","vertices","textPosition","landingLength","textOverride","dimensionStyleId"]),dimensionMultileader:fields(["id","type","layerId","color","linetype","lineweight","branches","textPosition","landingLength","textOverride","dimensionStyleId"]),leaderBranch:fields(["featureId","vertices"]),text:fields(["id","type","layerId","color","linetype","lineweight","insertionPoint","text","height","rotation","horizontalAlignment"]),region:fields(["id","type","layerId","color","linetype","lineweight","loops"]),hatch:fields(["id","type","layerId","color","linetype","lineweight","loops","pattern"]),hatchPattern:fields(["kind","name","angle","scale","origin"]),regionLoop:fields(["featureId","depth","parentIndex","edges"]),regionEdge:fields(["kind","featureId","start","end","center","majorAxis","radius","minorRadius","sweep","clockwise"])})
  ...
  function migrateDocument(document){if(document?.formatVersion===3){const source=copyValue(document);source.groups??={};source.nextGroupNumber??=1;source.blockDefinitions??={};source.layerOrder??=Object.keys(source.layers||{}).sort();return source}const source=copyValue(document),styleId=`ds_${source.id}_standard`,style={id:styleId,name:"Standard",...(source.dimensionStyle||DEFAULT_DIMENSION_STYLE)},objects={};for(const [id,record] of Object.entries(source.geometry?.objects||{}))objects[id]=record.type?.startsWith("dimension-")?{...record,dimensionStyleId:styleId}:record;delete source.dimensionStyle;return{...source,formatVersion:3,geometry:{objects},layerOrder:Object.keys(source.layers||{}).sort(),dimensionStyles:{[styleId]:style},dimensionStyleOrder:[styleId],currentDimensionStyleId:styleId,groups:{},nextGroupNumber:1,blockDefinitions:{}}}
  ...
  function validateDocument(value,{skipBlocks=false}={}) {
    ...
    function dimensionControls(record,label){for(const key of ["prefixOverride","suffixOverride"])if(record[key]!==undefined&&record[key]!==null&&(typeof record[key]!=="string"||record[key].length>64||/[\u0000-\u001f\u007f]/.test(record[key])))errors.push(`${label}: invalid ${key}`);const mode=record.toleranceMode??"none";if(!["none","symmetric","deviation"].includes(mode))errors.push(`${label}: invalid toleranceMode`);const upper=record.toleranceUpper??0,lower=record.toleranceLower??0;if(!Number.isFinite(upper)||!Number.isFinite(lower)||upper<0||lower<0||(mode==="symmetric"&&upper<=0)||(mode==="deviation"&&(upper<=0&&lower<=0)))errors.push(`${label}: invalid tolerance values`);const placement=record.textPositionMode??"automatic";if(!["automatic","manual"].includes(placement))errors.push(`${label}: invalid textPositionMode`);if(placement==="manual"){point(record.manualTextPosition,`${label} manual text position`);closedShape(record.manualTextPosition,V3_FIELDS.endpoint,`${label} manual text position`);identity(record.manualTextPosition?.featureId,`${label} manual text feature`)}else if(record.manualTextPosition!==undefined&&record.manualTextPosition!==null)errors.push(`${label}: automatic position must not store a manual point`)}
    ...
    if(!Array.isArray(value.layerOrder)||!isRecord(layers)||value.layerOrder.length!==Object.keys(layers).length||new Set(value.layerOrder).size!==value.layerOrder.length||value.layerOrder.some(id=>!has(layers,id)))errors.push("Invalid layer order")
    ...
    if (!isRecord(objects)) errors.push("Invalid object table")
      ...
      } else if(record.type==="dimension-leader"){
        closedShape(record,schema.dimensionLeader,"Leader");if(!Array.isArray(record.vertices)||record.vertices.length<2)errors.push("Leader: at least two vertices are required");else for(const vertex of record.vertices){point(vertex,"Leader vertex");closedShape(vertex,schema.endpoint,"Leader vertex");identity(vertex.featureId,"Leader vertex feature")}point(record.textPosition,"Leader text position");closedShape(record.textPosition,schema.endpoint,"Leader text position");identity(record.textPosition?.featureId,"Leader text feature");if(!Number.isFinite(record.landingLength)||record.landingLength<0)errors.push("Leader: invalid landing length");if(!has(styles||{},record.dimensionStyleId))errors.push("Leader: invalid dimensionStyleId");dimensionTextOverride(record.textOverride,"Leader")
      } else if(record.type==="dimension-multileader"){
        closedShape(record,schema.dimensionMultileader,"Multileader");if(!Array.isArray(record.branches)||!record.branches.length)errors.push("Multileader: at least one branch is required");else for(const branch of record.branches){closedShape(branch,schema.leaderBranch,"Multileader branch");identity(branch?.featureId,"Multileader branch feature");if(!Array.isArray(branch?.vertices)||branch.vertices.length<2)errors.push("Multileader branch: at least two vertices are required");else for(const vertex of branch.vertices){point(vertex,"Multileader vertex");closedShape(vertex,schema.endpoint,"Multileader vertex");identity(vertex.featureId,"Multileader vertex feature")}}point(record.textPosition,"Multileader text position");closedShape(record.textPosition,schema.endpoint,"Multileader text position");identity(record.textPosition?.featureId,"Multileader text feature");if(!Number.isFinite(record.landingLength)||record.landingLength<0)errors.push("Multileader: invalid landing length");if(!has(styles||{},record.dimensionStyleId))errors.push("Multileader: invalid dimensionStyleId");dimensionTextOverride(record.textOverride,"Multileader")
  ...
  function createStore({ document: initialDocument, initiallySaved = false } = {}) {
    ...
    const controller = window.DocumentController.createController({
      ...
      getCollections: document => ({ records: document.geometry.objects, groups:document.groups, blockDefinitions:document.blockDefinitions,layers: document.layers,
        settings: { units: document.units, currentLayerId: document.currentLayerId,layerOrder:document.layerOrder,dimensionStyles:document.dimensionStyles,dimensionStyleOrder:document.dimensionStyleOrder,currentDimensionStyleId:document.currentDimensionStyleId,nextGroupNumber:document.nextGroupNumber } }),
      assembleDocument: (baseDocument, collections) => ({
        ...
        currentLayerId: collections.settings.currentLayerId,layerOrder:collections.settings.layerOrder,
    ...
    const reader = Object.freeze({
      ...
      layers: () => Object.freeze(state.layerOrder.map(id=>state.layers[id])),
    ...
    const dimensionControlValues=(geometry,feature)=>Object.freeze({prefixOverride:geometry.prefixOverride??null,suffixOverride:geometry.suffixOverride??null,toleranceMode:geometry.toleranceMode??"none",toleranceUpper:geometry.toleranceUpper??0,toleranceLower:geometry.toleranceLower??0,textPositionMode:geometry.textPositionMode??"automatic",manualTextPosition:geometry.textPositionMode==="manual"&&geometry.manualTextPosition?feature(geometry.manualTextPosition):null})
    const recordGateway = Object.freeze({
      ...
      createLinearDimension(geometry){const feature=value=>({x:value?.x,y:value?.y,featureId:newId()});return freeze({id:newId(),type:"dimension-linear",layerId:currentDrawingLayerId(),...window.CaderactObjectProperties.BY_LAYER_PROPERTIES,mode:geometry.mode,firstPoint:feature(geometry.firstPoint),secondPoint:feature(geometry.secondPoint),dimensionLinePoint:feature(geometry.dimensionLinePoint),textOverride:geometry.textOverride??null,dimensionStyleId:geometry.dimensionStyleId||state.currentDimensionStyleId,...dimensionControlValues(geometry,feature)})},
      createAngularDimension(geometry){const feature=value=>({x:value?.x,y:value?.y,featureId:newId()});return freeze({id:newId(),type:"dimension-angular",layerId:currentDrawingLayerId(),...window.CaderactObjectProperties.BY_LAYER_PROPERTIES,firstRayPoint:feature(geometry.firstRayPoint),vertex:feature(geometry.vertex),secondRayPoint:feature(geometry.secondRayPoint),dimensionArcPoint:feature(geometry.dimensionArcPoint),textOverride:geometry.textOverride??null,dimensionStyleId:geometry.dimensionStyleId||state.currentDimensionStyleId,...dimensionControlValues(geometry,feature)})},
      createRadialDimension(geometry){const feature=value=>({x:value?.x,y:value?.y,featureId:newId()});return freeze({id:newId(),type:"dimension-radial",layerId:currentDrawingLayerId(),...window.CaderactObjectProperties.BY_LAYER_PROPERTIES,mode:geometry.mode,centerPoint:feature(geometry.centerPoint),dimensionPoint:feature(geometry.dimensionPoint),leaderPoint:feature(geometry.leaderPoint),textOverride:geometry.textOverride??null,dimensionStyleId:geometry.dimensionStyleId||state.currentDimensionStyleId,...dimensionControlValues(geometry,feature)})},
      createOrdinateDimension(geometry){const feature=value=>({x:value?.x,y:value?.y,featureId:newId()});return freeze({id:newId(),type:"dimension-ordinate",layerId:currentDrawingLayerId(),...window.CaderactObjectProperties.BY_LAYER_PROPERTIES,axis:geometry.axis,datumPoint:feature(geometry.datumPoint),featurePoint:feature(geometry.featurePoint),leaderPoint:feature(geometry.leaderPoint),textOverride:geometry.textOverride??null,dimensionStyleId:geometry.dimensionStyleId||state.currentDimensionStyleId,...dimensionControlValues(geometry,feature)})},
      createArcLengthDimension(geometry){const feature=value=>({x:value?.x,y:value?.y,featureId:newId()});return freeze({id:newId(),type:"dimension-arc-length",layerId:currentDrawingLayerId(),...window.CaderactObjectProperties.BY_LAYER_PROPERTIES,centerPoint:feature(geometry.centerPoint),startPoint:feature(geometry.startPoint),endPoint:feature(geometry.endPoint),dimensionArcPoint:feature(geometry.dimensionArcPoint),sweep:geometry.sweep,sourceRecordId:geometry.sourceRecordId,textOverride:geometry.textOverride??null,dimensionStyleId:geometry.dimensionStyleId||state.currentDimensionStyleId,...dimensionControlValues(geometry,feature)})},
      ...
      createLeader(geometry){const feature=value=>({x:value?.x,y:value?.y,featureId:newId()});return freeze({id:newId(),type:"dimension-leader",layerId:currentDrawingLayerId(),...window.CaderactObjectProperties.BY_LAYER_PROPERTIES,vertices:Object.freeze(geometry.vertices.map(feature)),textPosition:feature(geometry.textPosition),landingLength:geometry.landingLength,textOverride:geometry.textOverride,dimensionStyleId:geometry.dimensionStyleId||state.currentDimensionStyleId})},
      createMultileader(geometry){const feature=value=>({x:value?.x,y:value?.y,featureId:newId()});return freeze({id:newId(),type:"dimension-multileader",layerId:currentDrawingLayerId(),...window.CaderactObjectProperties.BY_LAYER_PROPERTIES,branches:Object.freeze(geometry.branches.map(branch=>Object.freeze({featureId:newId(),vertices:Object.freeze(branch.vertices.map(feature))}))),textPosition:feature(geometry.textPosition),landingLength:geometry.landingLength,textOverride:geometry.textOverride,dimensionStyleId:geometry.dimensionStyleId||state.currentDimensionStyleId})},
      ...
      setDimensionTextPosition(recordId, point) {
        const record=state.geometry.objects[recordId]
        if(!recordEditable(recordId))return Object.freeze({status:"record-layer-unavailable",recordId})
        if(!record||!DIMENSION_CONTROL_TYPES.has(record.type))return Object.freeze({status:"unsupported-record",recordId})
        const replacement=point===null
          ?{...record,textPositionMode:"automatic",manualTextPosition:null}
          :{...record,textPositionMode:"manual",manualTextPosition:{x:point?.x,y:point?.y,featureId:record.manualTextPosition?.featureId||newId()}}
        return this.replace(recordId,replacement)
      },
      removeAll(recordIds) {
      ...
      copyWithFreshIdentity(record) {
        ...
        if(record.type==="dimension-leader")return freeze({...record,id:newId(),vertices:record.vertices.map(vertex=>({...vertex,featureId:newId()})),textPosition:{...record.textPosition,featureId:newId()}})
        if(record.type==="dimension-multileader")return freeze({...record,id:newId(),branches:record.branches.map(branch=>({...branch,featureId:newId(),vertices:branch.vertices.map(vertex=>({...vertex,featureId:newId()}))})),textPosition:{...record.textPosition,featureId:newId()}})
    ...
    const groupGateway=Object.freeze({
      ...
      rename(groupId,name){const group=state.groups[groupId];if(!group)return Object.freeze({status:"missing-group",groupId});if(!validGroupName(name))return Object.freeze({status:"invalid-name"});if(Object.values(state.groups).some(candidate=>candidate.id!==groupId&&candidate.name.toLowerCase()===name.toLowerCase()))return Object.freeze({status:"duplicate-name"});if(name===group.name)return Object.freeze({status:"no-op",group});const replacement=freeze({...group,name}),transaction=controller.beginTransaction();try{transaction.replaceIn("groups",groupId,replacement);const outcome=transaction.publish();return Object.freeze({...outcome,group:replacement})}catch(error){if(transaction.isOpen)transaction.rollback();return Object.freeze({status:"commit-failed",message:error.message})}},
      addMembers(groupId,memberIds){const group=state.groups[groupId];if(!group)return Object.freeze({status:"missing-group",groupId});const requested=Array.from(new Set(Array.from(memberIds||[])));if(requested.some(id=>typeof id!=="string"||!has(state.geometry.objects,id)))return Object.freeze({status:"missing-member"});if(requested.some(id=>{const owner=reader.groupForRecord(id);return owner&&owner.id!==groupId}))return Object.freeze({status:"already-grouped"});const additions=requested.filter(id=>!group.memberIds.includes(id));if(!additions.length)return Object.freeze({status:"no-op",group});if(group.memberIds.length+additions.length>MAX_GROUP_MEMBERS)return Object.freeze({status:"member-limit"});const replacement=freeze({...group,memberIds:[...group.memberIds,...additions].sort()}),transaction=controller.beginTransaction();try{transaction.replaceIn("groups",groupId,replacement);const outcome=transaction.publish();return Object.freeze({...outcome,group:replacement,addedIds:Object.freeze(additions.sort())})}catch(error){if(transaction.isOpen)transaction.rollback();return Object.freeze({status:"commit-failed",message:error.message})}},
      removeMembers(groupId,memberIds){const group=state.groups[groupId];if(!group)return Object.freeze({status:"missing-group",groupId});const requested=new Set(Array.from(memberIds||[])),removed=group.memberIds.filter(id=>requested.has(id));if(!removed.length)return Object.freeze({status:"no-op",group});const memberIdsRemaining=group.memberIds.filter(id=>!requested.has(id)),transaction=controller.beginTransaction();try{let replacement=null;if(memberIdsRemaining.length<2)transaction.removeIn("groups",groupId);else{replacement=freeze({...group,memberIds:memberIdsRemaining});transaction.replaceIn("groups",groupId,replacement)}const outcome=transaction.publish();return Object.freeze({...outcome,group:replacement,removedIds:Object.freeze(removed),dissolved:replacement===null})}catch(error){if(transaction.isOpen)transaction.rollback();return Object.freeze({status:"commit-failed",message:error.message})}},
    ...
    const blockDefinitionGateway=Object.freeze({
      ...
      rename(definitionId,name){const current=state.blockDefinitions[definitionId];if(!current)return Object.freeze({status:"missing-definition"});if(typeof name!=="string"||name.trim()!==name||!name||name.length>MAX_BLOCK_NAME_LENGTH||/[\u0000-\u001f\u007f]/.test(name))return Object.freeze({status:"invalid-name"});if(Object.values(state.blockDefinitions).some(definition=>definition.id!==definitionId&&definition.name.toLowerCase()===name.toLowerCase()))return Object.freeze({status:"duplicate-name"});if(name===current.name)return Object.freeze({status:"no-op",definition:current});return blockDefinitionGateway.replace(definitionId,{name})},
      ...
      purgeUnused(){const reachable=new Set(),visit=id=>{if(reachable.has(id)||!has(state.blockDefinitions,id))return;reachable.add(id);for(const record of Object.values(state.blockDefinitions[id].records))if(record.type==="block-instance")visit(record.definitionId)};for(const record of Object.values(state.geometry.objects))if(record.type==="block-instance")visit(record.definitionId);const removed=Object.keys(state.blockDefinitions).filter(id=>!reachable.has(id)).sort();if(!removed.length)return Object.freeze({status:"no-op",definitionIds:Object.freeze([])});const transaction=controller.beginTransaction();try{for(const id of removed)transaction.removeIn("blockDefinitions",id);const outcome=transaction.publish();return Object.freeze({...outcome,definitionIds:Object.freeze(removed)})}catch(error){if(transaction.isOpen)transaction.rollback();return Object.freeze({status:"commit-failed",message:error.message})}},
    ...
    const layerGateway = Object.freeze({
      ...
      create(name, { makeCurrent = false } = {}) {
        ...
        transaction.replaceIn("settings", "layerOrder", [...state.layerOrder, layer.id])
      ...
      remove(layerId) {
        ...
        transaction.replaceIn("settings", "layerOrder", state.layerOrder.filter(id=>id!==layerId))
        return transaction.publish()
      },
      reorder(layerId,targetIndex){if(!has(state.layers,layerId))return Object.freeze({status:"unknown-layer",layerId});const from=state.layerOrder.indexOf(layerId),index=Math.max(0,Math.min(state.layerOrder.length-1,Math.trunc(Number(targetIndex))));if(from===index)return Object.freeze({status:"no-op",changes:Object.freeze([])});const order=state.layerOrder.filter(id=>id!==layerId);order.splice(index,0,layerId);const transaction=controller.beginTransaction();transaction.replaceIn("settings","layerOrder",order);return transaction.publish()},

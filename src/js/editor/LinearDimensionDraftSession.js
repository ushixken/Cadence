...
(() => {
  ...
  function create({createRecord,commitRecords,mode=null}){
    let points=[],candidate=null
    function preview(){if(points.length!==2||!candidate)return null;return Object.freeze({type:"dimension-linear",mode:mode||orientation(points[0],points[1],candidate),firstPoint:points[0],secondPoint:points[1],dimensionLinePoint:candidate,textOverride:null})}

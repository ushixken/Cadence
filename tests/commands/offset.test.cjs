...
test('Offset creates each Line side as a fresh record and stays in its persistent loop', async () => {
  ...
  const offset = b.read('modelReader.records()[1]'); assert.deepEqual({start:{x:offset.start.x,y:offset.start.y},end:{x:offset.end.x,y:offset.end.y}}, {start:{x:0,y:4},end:{x:20,y:4}});

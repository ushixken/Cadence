...
(() => {
  ...
  function canonicalRecord(record) {
    ...
    if(record.type==="dimension-linear")return {id:record.id,type:record.type,layerId:record.layerId,...properties(record),mode:record.mode,firstPoint:feature(record.firstPoint),secondPoint:feature(record.secondPoint),dimensionLinePoint:feature(record.dimensionLinePoint),textOverride:record.textOverride,dimensionStyleId:record.dimensionStyleId,...dimensionControls(record)}
    if(record.type==="dimension-angular")return {id:record.id,type:record.type,layerId:record.layerId,...properties(record),firstRayPoint:feature(record.firstRayPoint),vertex:feature(record.vertex),secondRayPoint:feature(record.secondRayPoint),dimensionArcPoint:feature(record.dimensionArcPoint),textOverride:record.textOverride,dimensionStyleId:record.dimensionStyleId,...dimensionControls(record)}
    if(record.type==="dimension-radial")return {id:record.id,type:record.type,layerId:record.layerId,...properties(record),mode:record.mode,centerPoint:feature(record.centerPoint),dimensionPoint:feature(record.dimensionPoint),leaderPoint:feature(record.leaderPoint),textOverride:record.textOverride,dimensionStyleId:record.dimensionStyleId,...dimensionControls(record)}
    if(record.type==="dimension-ordinate")return{id:record.id,type:record.type,layerId:record.layerId,...properties(record),axis:record.axis,datumPoint:feature(record.datumPoint),featurePoint:feature(record.featurePoint),leaderPoint:feature(record.leaderPoint),textOverride:record.textOverride,dimensionStyleId:record.dimensionStyleId,...dimensionControls(record)}
    if(record.type==="dimension-arc-length")return{id:record.id,type:record.type,layerId:record.layerId,...properties(record),centerPoint:feature(record.centerPoint),startPoint:feature(record.startPoint),endPoint:feature(record.endPoint),dimensionArcPoint:feature(record.dimensionArcPoint),sweep:record.sweep,sourceRecordId:record.sourceRecordId,textOverride:record.textOverride,dimensionStyleId:record.dimensionStyleId,...dimensionControls(record)}
    if(record.type==="dimension-center-mark")return{id:record.id,type:record.type,layerId:record.layerId,...properties(record),centerPoint:feature(record.centerPoint),sizePoint:feature(record.sizePoint),sourceRecordId:record.sourceRecordId,dimensionStyleId:record.dimensionStyleId}
    if(record.type==="dimension-center-line")return{id:record.id,type:record.type,layerId:record.layerId,...properties(record),firstPoint:feature(record.firstPoint),secondPoint:feature(record.secondPoint),extension:record.extension,sourceRecordIds:Array.from(record.sourceRecordIds),dimensionStyleId:record.dimensionStyleId}
    if(record.type==="dimension-leader")return{id:record.id,type:record.type,layerId:record.layerId,...properties(record),vertices:record.vertices.map(feature),textPosition:feature(record.textPosition),landingLength:record.landingLength,textOverride:record.textOverride,dimensionStyleId:record.dimensionStyleId}
    if(record.type==="dimension-multileader")return{id:record.id,type:record.type,layerId:record.layerId,...properties(record),branches:record.branches.map(branch=>({featureId:branch.featureId,vertices:branch.vertices.map(feature)})),textPosition:feature(record.textPosition),landingLength:record.landingLength,textOverride:record.textOverride,dimensionStyleId:record.dimensionStyleId}
      ...
      for (const item of items) {
        ...
        if (label === "record"||label==="definition record") {
          if (item.type === "line") {
            ...
            rejectUnknown(item,fields.dimensionLinear,"Linear dimension record");for(const key of ["firstPoint","secondPoint","dimensionLinePoint"]){if(!isRecord(item[key]))invalid(`Linear dimension ${key} must be an object`);rejectUnknown(item[key],fields.endpoint,`Linear dimension ${key}`)}if(item.manualTextPosition){if(!isRecord(item.manualTextPosition))invalid("Linear dimension manual text position must be an object");rejectUnknown(item.manualTextPosition,fields.endpoint,"Linear dimension manual text position")}
          } else if(!legacy&&item.type==="dimension-angular"){
            rejectUnknown(item,fields.dimensionAngular,"Angular dimension record");for(const key of ["firstRayPoint","vertex","secondRayPoint","dimensionArcPoint"]){if(!isRecord(item[key]))invalid(`Angular dimension ${key} must be an object`);rejectUnknown(item[key],fields.endpoint,`Angular dimension ${key}`)}if(item.manualTextPosition){if(!isRecord(item.manualTextPosition))invalid("Angular dimension manual text position must be an object");rejectUnknown(item.manualTextPosition,fields.endpoint,"Angular dimension manual text position")}
          } else if(!legacy&&item.type==="dimension-radial"){
            rejectUnknown(item,fields.dimensionRadial,"Radial dimension record");for(const key of ["centerPoint","dimensionPoint","leaderPoint"]){if(!isRecord(item[key]))invalid(`Radial dimension ${key} must be an object`);rejectUnknown(item[key],fields.endpoint,`Radial dimension ${key}`)}if(item.manualTextPosition){if(!isRecord(item.manualTextPosition))invalid("Radial dimension manual text position must be an object");rejectUnknown(item.manualTextPosition,fields.endpoint,"Radial dimension manual text position")}
          } else if(!legacy&&!version2&&item.type==="dimension-ordinate"){
            rejectUnknown(item,fields.dimensionOrdinate,"Ordinate dimension record");for(const key of ["datumPoint","featurePoint","leaderPoint"]){if(!isRecord(item[key]))invalid(`Ordinate dimension ${key} must be an object`);rejectUnknown(item[key],fields.endpoint,`Ordinate dimension ${key}`)}if(item.manualTextPosition){if(!isRecord(item.manualTextPosition))invalid("Ordinate dimension manual text position must be an object");rejectUnknown(item.manualTextPosition,fields.endpoint,"Ordinate dimension manual text position")}
          } else if(!legacy&&!version2&&item.type==="dimension-arc-length"){
            rejectUnknown(item,fields.dimensionArcLength,"Arc-length dimension record");for(const key of ["centerPoint","startPoint","endPoint","dimensionArcPoint"]){if(!isRecord(item[key]))invalid(`Arc-length dimension ${key} must be an object`);rejectUnknown(item[key],fields.endpoint,`Arc-length dimension ${key}`)}if(item.manualTextPosition){if(!isRecord(item.manualTextPosition))invalid("Arc-length dimension manual text position must be an object");rejectUnknown(item.manualTextPosition,fields.endpoint,"Arc-length dimension manual text position")}
          } else if(!legacy&&!version2&&item.type==="dimension-center-mark"){
            rejectUnknown(item,fields.dimensionCenterMark,"Center mark record");for(const key of ["centerPoint","sizePoint"]){if(!isRecord(item[key]))invalid(`Center mark ${key} must be an object`);rejectUnknown(item[key],fields.endpoint,`Center mark ${key}`)}
          } else if(!legacy&&!version2&&item.type==="dimension-center-line"){
            rejectUnknown(item,fields.dimensionCenterLine,"Center line record");for(const key of ["firstPoint","secondPoint"]){if(!isRecord(item[key]))invalid(`Center line ${key} must be an object`);rejectUnknown(item[key],fields.endpoint,`Center line ${key}`)}
          } else if(!legacy&&!version2&&item.type==="dimension-leader"){
            rejectUnknown(item,fields.dimensionLeader,"Leader record");if(!Array.isArray(item.vertices))invalid("Leader vertices must be an array");for(const vertex of item.vertices){if(!isRecord(vertex))invalid("Leader vertex must be an object");rejectUnknown(vertex,fields.endpoint,"Leader vertex")}if(!isRecord(item.textPosition))invalid("Leader text position must be an object");rejectUnknown(item.textPosition,fields.endpoint,"Leader text position")
          } else if(!legacy&&!version2&&item.type==="dimension-multileader"){
            rejectUnknown(item,fields.dimensionMultileader,"Multileader record");if(!Array.isArray(item.branches))invalid("Multileader branches must be an array");for(const branch of item.branches){if(!isRecord(branch))invalid("Multileader branch must be an object");rejectUnknown(branch,fields.leaderBranch,"Multileader branch");if(!Array.isArray(branch.vertices))invalid("Multileader branch vertices must be an array");for(const vertex of branch.vertices){if(!isRecord(vertex))invalid("Multileader vertex must be an object");rejectUnknown(vertex,fields.endpoint,"Multileader vertex")}}if(!isRecord(item.textPosition))invalid("Multileader text position must be an object");rejectUnknown(item.textPosition,fields.endpoint,"Multileader text position")

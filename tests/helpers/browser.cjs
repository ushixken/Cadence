...
async function browser({ commands = true, realRenderer = false, gpu } = {}) {
  ...
  const fileNewButton = new Element('button'); const fileOpenButton = new Element('button'); const fileSaveButton = new Element('button'); const fileSaveAsButton = new Element('button'); const fileImportDxfButton = new Element('button'); const fileExportDxfButton = new Element('button');
  ...
  const snapWrap = new Element(); const snapTrigger = new Element('button'); const osnapMenuTrigger=new Element('button');const snapMenu = new Element(); const snapEnabled = new Element('input'); const insertionSnapOption = new Element('input'); const extensionTrackingOption = new Element('input'); const snapDependent = new Element();
  const gridVisibleButton=new Element('button'),dynamicInputButton=new Element('button'),gridVisibleOption=new Element('input'),gridSnapOption=new Element('input'),polarEnabledOption=new Element('input'),polarIncrementOption=new Element('select'),trackEnabledOption=new Element('input'),dynamicInputEnabledOption=new Element('input');
  const gridMenu=new Element(),polarMenu=new Element(),trackMenu=new Element(),dynamicInputMenu=new Element(),gridMenuTrigger=new Element('button'),polarMenuTrigger=new Element('button'),trackMenuTrigger=new Element('button'),dynamicInputMenuTrigger=new Element('button');
  ...
  const layersList = new Element(); const layerCreateButton = new Element('button'); const layerAssignButton = new Element('button'); const layerUnisolateButton = new Element('button'); layerUnisolateButton.hidden=true;
  const editorWorkspace=new Element();const layersPanel=new Element('aside');const rightDockResize=new Element('button');editorWorkspace.getBoundingClientRect=()=>({width:1000,height:600,left:0,top:0,right:1000,bottom:600});layersPanel.parent=editorWorkspace;rightDockResize.parent=layersPanel;
  const layersTab=new Element('button'),groupsTab=new Element('button'),blocksTab=new Element('button'),propertiesTab=new Element('button'),layersView=new Element('section'),groupsView=new Element('section'),blocksView=new Element('section'),propertiesView=new Element('section'),propertiesContent=new Element();
  const groupsList=new Element(),groupCreateButton=new Element('button'),blocksList=new Element(),blockCreateButton=new Element('button'),blockPurgeButton=new Element('button');layersTab.hidden=false;groupsTab.hidden=false;blocksTab.hidden=false;propertiesTab.hidden=false;layersView.hidden=false;groupsView.hidden=true;blocksView.hidden=true;propertiesView.hidden=true;propertiesContent.hidden=false;
  const blockEditContext=new Element('section'),blockEditName=new Element('span'),blockEditSave=new Element('button'),blockEditDiscard=new Element('button');blockEditContext.hidden=true;
  ...
  snapTrigger.parent=snapWrap;osnapMenuTrigger.parent=snapWrap;snapMenu.parent=snapWrap;snapEnabled.parent=snapMenu;insertionSnapOption.parent=snapDependent;snapDependent.parent=snapMenu;gridMenuTrigger.parent=gridMenu;polarMenuTrigger.parent=polarMenu;trackMenuTrigger.parent=trackMenu;dynamicInputMenuTrigger.parent=dynamicInputMenu;gridVisibleOption.parent=gridMenu;gridSnapOption.parent=gridMenu;polarEnabledOption.parent=polarMenu;polarIncrementOption.parent=polarMenu;trackEnabledOption.parent=trackMenu;extensionTrackingOption.parent=trackMenu;dynamicInputEnabledOption.parent=dynamicInputMenu;
  osnapMenuTrigger.setAttribute('aria-controls','snap-menu');gridMenuTrigger.setAttribute('aria-controls','grid-drafting-menu');polarMenuTrigger.setAttribute('aria-controls','polar-drafting-menu');trackMenuTrigger.setAttribute('aria-controls','track-drafting-menu');dynamicInputMenuTrigger.setAttribute('aria-controls','dynamic-input-drafting-menu');for(const trigger of [osnapMenuTrigger,gridMenuTrigger,polarMenuTrigger,trackMenuTrigger,dynamicInputMenuTrigger])trigger.setAttribute('aria-expanded','false');
  ...
  for(const el of [snapWrap,snapTrigger,osnapMenuTrigger,snapMenu,snapEnabled,insertionSnapOption,extensionTrackingOption,snapDependent,gridVisibleButton,dynamicInputButton,gridVisibleOption,gridSnapOption,polarEnabledOption,polarIncrementOption,trackEnabledOption,dynamicInputEnabledOption,gridMenu,polarMenu,trackMenu,dynamicInputMenu,gridMenuTrigger,polarMenuTrigger,trackMenuTrigger,dynamicInputMenuTrigger,unitsWrap,unitsTrigger,unitsMenu,unitValue,...unitOptions]) { el.owner=document; if(!el.parent) el.parent=document; }
  snapEnabled.checked=true; snapEnabled.dataset.snapMode='object'; insertionSnapOption.checked=true; insertionSnapOption.dataset.snapMode='insertion'; extensionTrackingOption.checked=true;gridVisibleOption.checked=true;trackEnabledOption.checked=true;dynamicInputEnabledOption.checked=true;polarIncrementOption.value='45'; unitValue.textContent='mm'; unitOptions[0].classList.add('is-selected');
  for (const el of [layersList,layerCreateButton,layerAssignButton,layerUnisolateButton,groupsList,groupCreateButton,blocksList,blockCreateButton,blockPurgeButton,blockEditContext,blockEditName,blockEditSave,blockEditDiscard,layersTab,groupsTab,blocksTab,propertiesTab,layersView,groupsView,blocksView,propertiesView,propertiesContent,contextMenu,gridSnapButton,orthoButton,polarButton,trackButton]) { el.parent=document; el.owner=document; }
  propertiesContent.parent=propertiesView;layersList.parent=layersView;groupsList.parent=groupsView;blocksList.parent=blocksView;
  ...
  document.querySelector = selector => ({ canvas, '.viewport': viewportHost,'.editor-workspace':editorWorkspace,'.layers-panel':layersPanel,'#right-dock-resize':rightDockResize,'#grid-visible-toggle':gridVisibleButton, '#grid-snap-toggle': gridSnapButton, '#ortho-toggle': orthoButton, '#polar-toggle': polarButton, '#track-toggle': trackButton,'#dynamic-input-toggle':dynamicInputButton,'#grid-visible-option':gridVisibleOption,'#grid-snap-option':gridSnapOption,'#polar-enabled-option':polarEnabledOption,'#polar-increment-option':polarIncrementOption,'#track-enabled-option':trackEnabledOption,'#dynamic-input-enabled-option':dynamicInputEnabledOption,'#grid-drafting-menu':gridMenu,'#polar-drafting-menu':polarMenu,'#track-drafting-menu':trackMenu,'#dynamic-input-drafting-menu':dynamicInputMenu,'#snap-menu':snapMenu, '#command-input': input, '#command-suggestions': suggestions, '#command-history': commandHistory, '#command-prompt': commandPrompt, '#command-name': commandName, '#undo-button': undoButton, '#redo-button': redoButton, '#file-new': fileNewButton, '#file-open': fileOpenButton, '#file-save': fileSaveButton, '#file-save-as': fileSaveAsButton, '#file-import-dxf': fileImportDxfButton, '#file-export-dxf': fileExportDxfButton, '.file-menu': fileMenu, '.file-menu-trigger': fileMenuTrigger, '#file-menu-actions': fileMenuDropdown, '.tools-menu':toolsMenu,'.tools-menu-trigger':toolsMenuTrigger,'#measure-menu-actions':measureMenuDropdown, '.snap-trigger': snapTrigger, '.snap-menu': snapMenu, '#snap-enabled': snapEnabled, '#extension-tracking-enabled':extensionTrackingOption, '.snap-dependent': snapDependent, '.units-control': unitsTrigger, '.units-menu': unitsMenu, '[data-unit-value]': unitValue, '#layers-list': layersList, '#layer-create': layerCreateButton, '#layer-assign': layerAssignButton, '#layer-unisolate':layerUnisolateButton, '#groups-list':groupsList, '#group-create':groupCreateButton, '#blocks-list':blocksList, '#block-create':blockCreateButton, '#block-purge':blockPurgeButton, '#block-edit-context':blockEditContext, '#block-edit-name':blockEditName, '#block-edit-save':blockEditSave, '#block-edit-discard':blockEditDiscard, '#sidebar-layers-tab':layersTab, '#sidebar-groups-tab':groupsTab, '#sidebar-blocks-tab':blocksTab, '#sidebar-properties-tab':propertiesTab, '#layers-panel-view':layersView, '#groups-panel-view':groupsView, '#blocks-panel-view':blocksView, '#properties-panel-view':propertiesView, '#properties-content':propertiesContent, '#editor-context-menu': contextMenu })[selector];
  ...
  const context = vm.createContext({ window, document, crypto: require('node:crypto').webcrypto, navigator: { gpu }, HTMLElement: Element,
    ...
    requestAnimationFrame: fn => frames.push(fn), setTimeout: setTimer, clearTimeout: clearTimer, TextEncoder, console: { info() {}, warn() {} },
  });
  ...
  load('src/js/geometry/HatchPatterns.js');
  ...
  load('src/js/geometry/DirectEditingPlanner.js');
  ...
  if (commands) load('src/js/editor/ColorPopover.js');
  if (commands) load('src/js/editor/layers-panel.js');
  if (commands) load('src/js/editor/groups-panel.js');
  if (commands) load('src/js/editor/blocks-panel.js');
  if (commands) load('src/js/editor/block-edit-context.js');
  ...
  return { window, document, viewportHost,editorWorkspace,layersPanel,rightDockResize, contextMenu, gridVisibleButton,gridSnapButton, orthoButton, polarButton, trackButton,dynamicInputButton,snapTrigger,osnapMenuTrigger,snapEnabled,insertionSnapOption,gridVisibleOption,gridSnapOption,polarEnabledOption,polarIncrementOption,trackEnabledOption,extensionTrackingOption,dynamicInputEnabledOption,gridMenu,polarMenu,snapMenu,trackMenu,dynamicInputMenu,gridMenuTrigger,polarMenuTrigger,trackMenuTrigger,dynamicInputMenuTrigger, get canvas() { return canvas; }, input, suggestions, commandHistory, commandPrompt, undoButton, redoButton, fileNewButton, fileOpenButton, fileSaveButton, fileSaveAsButton, fileImportDxfButton, fileExportDxfButton, fileMenu, fileMenuTrigger, fileMenuDropdown, toolsMenu,toolsMenuTrigger,measureMenuDropdown,measureMenuItems, editMenuTrigger, unitsTrigger, unitsMenu, unitValue, unitOptions, layersList, layerCreateButton, layerAssignButton, layerUnisolateButton, groupsList,groupCreateButton,blocksList,blockCreateButton,blockPurgeButton,blockEditContext,blockEditName,blockEditSave,blockEditDiscard,layersTab,groupsTab,blocksTab,propertiesTab,layersView,groupsView,blocksView,propertiesView,propertiesContent,context, run, load, emit, renders, sizes, fakeRenderer, drawCalls, observerStats,
